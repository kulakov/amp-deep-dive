  ...
  return <section className="py-24 px-6 border-b border-callout-border bg-background relative z-10">
      ...
      <div className="max-w-3xl mx-auto mb-12">
        <div className="space-y-6">
          ...
          <p className="text-lg font-body leading-relaxed">Мы собираем в одном месте людей, которым точно захочется искренне и глубоко пообщаться. Искать таких сложно. Вы можете делать это сами, но на это вечно не хватает времени. А еще непонятно, где их брать. Поэтому надо собрать их в кучу. Вот мы и собрали.</p>
        </div>
      </div>
      ...
      <div className="md:hidden overflow-x-auto pb-4 -mx-6 px-6">
        ...
        width: 'max-content'
          ...
          {galleryPhotos.map((item, i) => <div key={i} className="w-32 flex-shrink-0 bg-white p-2 shadow-lg cursor-pointer" onClick={() => openLightbox(i)}>
      ...
      <div ref={galleryRef} className="hidden md:block relative h-[450px] max-w-6xl mx-auto">
        ...
        return <div key={i} className={`absolute ${pos.size} bg-white p-2 shadow-xl cursor-pointer transition-all duration-300 ease-out group
                ${isVisible ? "opacity-100" : "opacity-0 translate-y-8"}`} style={{
      ...
      {/* Lightbox */}
      {lightboxIndex !== null && <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center" onClick={closeLightbox}>
          {/* Image */}
          <div className="relative max-w-[90vw] max-h-[85vh] bg-white p-3" onClick={e => e.stopPropagation()}>
            <button className="absolute top-2 right-2 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 transition-colors" onClick={e => {
          e.stopPropagation();
          closeLightbox();
        }}>
              <X size={20} />
            ...
            <img src={galleryPhotos[lightboxIndex].img} alt={galleryPhotos[lightboxIndex].caption} className="max-w-full max-h-[80vh] object-contain" />
          </div>

          {/* Next button */}
          <button className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 text-white/80 hover:text-white transition-colors p-2" onClick={e => {
        e.stopPropagation();
        goNext();
      }}>
      ...
      <div className="max-w-3xl mx-auto space-y-20">
        ...
        <div className="space-y-6">
          <p className="text-lg font-body leading-relaxed">
            AmpCamp — это место, где <strong>технический директор</strong> и <strong>учительница литературы</strong> могут обнаружить, 
            что в их <strong>тревоге о будущем</strong> есть много общего, хотя внешне их жизни не похожи.
          </p>
          <p className="text-lg font-body leading-relaxed">
        ...
        <div id="not-conference" className="relative -mx-6 md:-mx-12 px-6 md:px-12 py-16 my-8">
          ...
          <div className="absolute inset-0 -z-10 overflow-hidden bg-highlight/10" style={{
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              items: "дискуссия про теорему Гёделя, семинар про то, что такое ответственность, дискуссия про психологию морали, доклад про гравюры Дюрера"
            }, {
              tag: "телесные",
              items: "йога-зарядка, мастер-класс по джиу-джитсу, турнир по водному поло, рейв в бассейне"
            }, {
              tag: "эмоциональные",
              items: "мастер-класс по комплиментам, ASMR-сессия, рассказ про школу приемных родителей, семинар по написанию завещаний, cuddle-party"
            }, {
              tag: "творческие",
              items: "мастер-класс по stop motion анимации, фотография на консервную банку, музыкальный джем, пение хасидских нигунов"
            }, {
              tag: "игры и баловство",
              items: "комедийная импровизация, турнир по «шляпе», стратегическая игра в ООН, сеанс аукционной магии, художественный LARP"
            }].map(cat => <div key={cat.tag}>
        ...
        <div className="relative -mx-6 md:-mx-12 px-6 md:px-12 py-8">
          ...
          <div className="text-foreground">
            ...
            <div className="md:hidden overflow-x-auto overflow-y-visible -mx-6 px-6 -mb-4">
              <div className="flex gap-4 pb-8" style={{
              width: 'max-content'
            }}>
                {[{
                name: "Google game",
            ...
            <div className="hidden md:block relative h-[500px]">
              ...
              width: "w-64",
              img: formatFuckupNight
            }].map(card => <div key={card.name} className={`absolute ${card.width} bg-background text-foreground shadow-2xl cursor-pointer transition-all duration-300`} style={{
