...
const Participants = () => {
  const participants = [{
    ...
    name: "Диана",
    description: "Юристка, переводит с языка законов на tech-язык и обратно. Изучает духи и ведёт блог о запахах.",
    ...
    name: "Дима",
    description: "Бросил алготрейдинг ради предпринимательства, а потом разумный бизнес ради биотеха. Учит второй концерт Рахманинова и любит игры с ракетками.",
    insight: "«Психолог говорит, что при избыточном оптимизме иногда полезно опираться на негативные эмоции. Стараюсь фокусироваться на тревоге, страхе, стыде»."
  }, {
    name: "Федя",
    ...
    name: "Оксана",
    description: "PhD по биологии, продюсерка онлайн-курсов, живет на Ко Пхангане и работает с российскими студентами в режиме +4 часа к московскому времени.",
  ...
  return <section ref={sectionRef} className="py-12 px-6 bg-background">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <blockquote className="text-2xl md:text-3xl font-display italic">«А такие, как я, участвуют?»</blockquote>
        </div>

        {/* Mobile carousel - stacked deck */}
        <div className="md:hidden relative h-[400px] flex items-center justify-center">
          <div ref={cardRef} className="relative w-full max-w-[300px] h-[350px] touch-pan-y" onTouchStart={e => {
          ...
          handleDragMove(e.touches[0].clientX);
        }} onTouchEnd={handleDragEnd} onMouseDown={e => handleDragStart(e.clientX)} onMouseMove={e => handleDragMove(e.clientX)} onMouseUp={handleDragEnd} onMouseLeave={handleDragEnd}>
            ...
            return <div key={i} className="absolute inset-0 bg-[#FFFEF5] border border-foreground/20 p-6 shadow-lg" style={{
            ...
            {/* Active card */}
            <div className="absolute inset-0 bg-[#FFFEF5] border border-foreground/20 p-6 shadow-xl transition-transform duration-200 cursor-pointer" style={{
        ...
        <div className="hidden md:block relative h-[1100px]">
          {participants.map((person, i) => <div key={i} className="absolute w-[30%] bg-[#FFFEF5] border border-foreground/20 p-6 shadow-lg cursor-pointer transition-all duration-300 group overflow-visible" style={{
          ...
          e.currentTarget.style.color = '';
            ...
            color: 'inherit'
              ...
              <p className="text-sm mb-4 leading-relaxed transition-colors duration-300" style={{
            color: 'inherit'
          }}>{person.description}</p>
              <p className="text-sm italic leading-relaxed transition-colors duration-300" style={{
            color: 'inherit'
          }}>{person.insight}</p>
            </div>)}
        </div>
